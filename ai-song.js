...
<textarea class="field" id="aiLyrics" rows="5" style="height:126px;min-height:126px;max-height:126px;overflow-y:auto;resize:none" placeholder="歌词会出现在这里，也可自己改"></textarea>
...
let generatedBlob=null,generatedKind='vocal';
...
$('#makeLyrics').onclick=async()=>{
  ...
  btn.disabled=true;status('AI 正在写歌词…');
  try{
    ...
    status('歌词已生成，下面生成歌曲。',true);
  }catch(e){status('歌词失败：'+(e.message||e))}finally{btn.disabled=false}
};
...
async function generateVocalServer(lyrics,style,mood,voice){
  ...
  try{
    ...
    if(!blob.size) throw new Error('人声通道返回空音频');
    return blob;
  }finally{clearTimeout(timer)}
}
async function generateInstrumental(style,mood){
  const g=await ensureGuest();status('正在用本站纯音乐通道生成…');
  ...
  try{
    ...
    if(!r.ok){let msg='纯音乐通道不可用';try{const j=await r.json();msg=j?.message||j?.error||msg}catch{}throw new Error(msg)}
    const blob=await r.blob();if(!blob.size)throw new Error('纯音乐通道返回空音频');return blob;
  }finally{clearTimeout(timer)}
}
...
async function makeFullSong(){
  ...
  if(!isInstrumental&&lyrics.length<40){status('请先点「生成歌词」或自己贴满歌词');return}
  ...
  try{
    ...
    status(isInstrumental?'纯音乐已生成，请试听。':'歌曲已生成，请试听。',true);
...
$('#saveGenerated').onclick=async()=>{
  ...
  try{
    ...
    try{const meta=await submitTrack(g,{title,artist,genre:$('#aiStyle').value,description:`AI原创 · ${generatedKind==='vocal'?'人声歌曲':'纯音乐'} · ${$('#aiMood').value}`,storage_path:path});if(typeof window.AIMUSIC_GUEST_AWARD==='function')await window.AIMUSIC_GUEST_AWARD('upload',meta.id)}
...
if(ordinarySubmit)ordinarySubmit.onclick=async()=>{
  ...
  ordinarySubmit.disabled=true;msg('正在上传并登记后台待审核…');
  try{
    const g=await ensureGuest(),ext=(f.name.split('.').pop()||'audio').toLowerCase().replace(/[^a-z0-9]/g,'');const path=`submissions/${Date.now()}-${crypto.randomUUID()}.${ext}`;
