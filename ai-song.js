...
<p class="notice">歌曲生成使用 DiffRhythm2 免费 GPU 通道。公共免费算力可能排队或暂时休眠；失败时不会伪造结果，可稍后重试。</p>
...
async function makeFullSong(){
 ...
 status('正在连接免费歌曲生成 GPU…首次唤醒可能需要等待。');
 try{
  ...
  const local=URL.createObjectURL(generatedBlob); $('#generatedAudio').src=local; $('#generatedBox').classList.remove('hidden'); status('完整歌曲生成成功 ✓ 先试听，满意后保存审核。',true);
...
$('#saveGenerated').onclick=async()=>{
 ...
 try{
  ...
  const meta=await db.from('aimusic_tracks').insert({title,artist,genre:$('#aiStyle').value,description:`AI原创 · ${$('#aiMood').value} · ${$('#aiVoice').value}`,storage_path:path,status:'pending',plays:0}); if(meta.error)throw meta.error;
  status('保存成功 ✓ 已进入待审核。审核通过后进入公共曲库，并可同步到 Telegram 频道。',true);
